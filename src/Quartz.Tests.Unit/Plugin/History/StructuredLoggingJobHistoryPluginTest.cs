...
public class StructuredLoggingJobHistoryPluginTest
{
    ...
    public async Task TestJobFailedMessage()
    {
        ...
        Assert.That(loggerProvider.Entries.Count, Is.EqualTo(1));
        Assert.That(loggerProvider.Entries[0].Level, Is.EqualTo(Microsoft.Extensions.Logging.LogLevel.Warning));
    ...
    internal sealed class LogEntry
    {
        ...
        public LogEntry(Microsoft.Extensions.Logging.LogLevel level, string message, Exception exception)
        {
            Level = level;
            Message = message;
            Exception = exception;
