...
public class UpdateTriggerTest
{
    ...
    public async Task CronTrigger_WhenExistingTriggerIsSimple_SwitchesTypeToCron()
    {
        ...
        // Existing trigger type is SIMPLE (we're switching from Simple to Cron)
        var selectTypeReader = A.Fake<DbDataReader>();
        A.CallTo(() => selectTypeReader.ReadAsync(CancellationToken.None))
            .Returns(true);
        A.CallTo(() => selectTypeReader[AdoConstants.ColumnTriggerType])
            .Returns(AdoConstants.TriggerTypeSimple);
        ...
        A.CallTo(() => dbProvider.CreateCommand()).ReturnsLazily(() =>
            ++createCommandCallCount == 1 ? selectTypeCommand : dbCommand);
        ...
        var triggerTypeParam = dataParameterCollectionOutputs
            .Select(x => x as IDataParameter)
            .FirstOrDefault(x => x?.ParameterName == "triggerType");
        Assert.That(triggerTypeParam?.Value, Is.EqualTo(AdoConstants.TriggerTypeCron), "triggerType parameter should be set to CRON");

        // Assert: cron expression should be set (from InsertExtendedTriggerProperties for CRON)
        var cronExpressionParam = dataParameterCollectionOutputs
            .Select(x => x as IDataParameter)
            .FirstOrDefault(x => x?.ParameterName == "triggerCronExpression");
        Assert.That(cronExpressionParam, Is.Not.Null, "triggerCronExpression parameter should have been set during CRON trigger insertion");
        Assert.That(cronExpressionParam!.Value, Is.EqualTo("0 15 23 * * ?"));
    }
}