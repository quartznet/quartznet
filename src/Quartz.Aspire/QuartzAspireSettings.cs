...
public sealed class QuartzAspireSettings
{
    ...
    /// <remarks>
    /// <para>
    /// The same three-valued decision <c>AdoJobStoreOptions.SchemaProvisioning</c> and
    /// <c>IPersistentStoreBuilder.ProvisionSchema()</c> speak, said from the Aspire section instead:
    ...
    /// <c>AddQuartzPersistentStore</c> call: <see cref="Quartz.SchemaProvisioning.CreateIfMissing"/>
    /// under <c>Development</c>, and nothing said everywhere else. The two halves of that are the same
    ...
    /// wants to say once, in the shared <c>Aspire:Quartz</c> section, rather than per environment.
    /// <see cref="Quartz.SchemaProvisioning.CreateIfMissing"/> only ever creates: no object is altered
    /// and none is dropped, so it cannot turn a mis-typed <see cref="TablePrefix"/> into data loss, and
    /// it is equally not an upgrade — <c>database/migrations/</c> is still what moves a schema forward.
    /// <see cref="Quartz.SchemaProvisioning.None"/> is the one thing the old <see langword="bool"/>
    ...
    /// <see cref="Quartz.SchemaProvisioning.Validate"/> here means "leave it alone": it is what an
    /// unconfigured store already holds, so writing it changes nothing, and it is how a
    /// <c>Development</c> application declines the creating default.
    /// </para>
    /// </remarks>
