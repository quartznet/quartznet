...
/// </summary>
/// <remarks>
/// Every callback leads with the <see cref="ITrigger" /> it is about, and every callback is told which
/// scheduler is calling it: a listener reaches the scheduler it serves through its execution context,
/// or as a second argument when there is no execution. Only <see cref="TriggerMisfired" /> is of the
/// second kind — a misfire is noticed rather than executed, so it has no context to carry one.
/// <para>
...
public interface ITriggerListener
{
    ...
    /// <see cref="IJobExecutionContext" /> here to reach it through. It comes after the trigger, as
    /// the context does on every other notification in this interface.
    /// <para>
    /// The default implementation does nothing.
    /// </para>
    /// </remarks>
