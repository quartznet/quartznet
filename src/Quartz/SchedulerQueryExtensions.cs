...
/// <remarks>
/// <para>
/// Reading has two altitudes on purpose, and this is the lower one. <see cref="IScheduler" />'s
/// <c>Query*</c> members take a query record — a filter, a page, an optional total count — and
/// answer with headers rich enough to render a listing. The calls here are the questions that need
/// none of that: they name no record, take no page, and answer with the bare keys and names a
/// caller who already knows what it wants is asking for. Neither family is the other's
/// leftovers — the query members cannot be as short as these, and these cannot page.
/// </para>
/// <para>
/// The price of the shortness is that every <c>Get*</c> listing here enumerates its whole result.
/// There is no paging: a scheduler with many jobs, triggers or groups pays for all of them on each
/// call. Use the query member with <see cref="PagedQuery.Skip" /> and <see cref="PagedQuery.Take" />
/// when the result may be large.
/// </para>
/// <para>
/// What does not live here is a shorthand that saves only the <c>new</c>:
/// <c>QueryJobs(new JobQuery())</c> is not worth an overload, because the record it names is the
/// query API's whole point. <see cref="QueryTriggersInError" /> stays because it is a preset rather
/// than a synonym — it knows the filter, and "which triggers are broken?" is a question, where "the
/// jobs, unfiltered" is just a call with an argument left out.
/// </para>
/// </remarks>
public static class SchedulerQueryExtensions
{
    ...
    /// <seealso cref="IScheduler.ResetTriggersFromErrorState(GroupMatcher{TriggerKey}, CancellationToken)" />
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        JobGroupQuery query = new() { Name = NameMatcher.NameEquals(groupName), Paused = true, Take = 1 };
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        TriggerGroupQuery query = new() { Name = NameMatcher.NameEquals(groupName), Paused = true, Take = 1 };
