...
public abstract class JobStoreSupport : AdoConstants, IJobStore, INextVersionJobStore
{
            ...
            if (UseDBLocks)
            {
                if (Delegate is SqlServerDelegate)
                {
                    if (SelectWithLockSQL == null)
                    {
                        ...
                        Log.InfoFormat("Detected usage of SqlServerDelegate - defaulting 'selectWithLockSQL' to '" + DefaultLockSql + "'.", TablePrefix);
    ...
    private async Task ProbeForOptionalColumns(CancellationToken cancellationToken)
    {
        ...
        {
            Log.Warn("Columns PREFERRED_NODE / PREFERRED_NODE_AUTO not found in triggers table. " +
                "Preferred node (node affinity) is not available. " +
                "Run the schema migration to add these columns for full support.");
        }
        else if (InstanceId != null)
        {
            ...
            // No instance-id value is reserved: the auto-claim flag lives in its own column,
            // so node names are stored verbatim and can never collide with an internal marker.
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (nvd.HasMisfireOriginalFireTimeColumn)
        {
        ...
        await nvd.UpdateMisfiredTrigger(conn, trig, newState, misfireOrigFireTime, cancellationToken).ConfigureAwait(false);
            ...
            if (existingCal)
            {
                ...
                if (updateTriggers)
                {
                    ...
                    foreach (IOperableTrigger trigger in triggers)
                    {
                        ...
                        if (triggerState.Equals(StateDeleted))
                        {
                            continue;
                        }
            ...
            try
            {
                ...
                foreach (var result in results)
                {
                    ...
                    catch (Exception e)
                    {
                        try
                        {
                            Log.ErrorException("Error retrieving job, setting trigger state to ERROR.", e);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Auto-pin: when preferred node is the "*" sentinel, claim the trigger by assigning this
        // node's instance id and flagging it as auto-claimed. When it is some OTHER node's id and
        // already auto-claimed, that node was stale or dead at acquisition time (the acquisition
        // SQL only releases another node's pin via the liveness fallback), so steal the pin —
        // sticky failover converges to a live node.
        // The write is a compare-and-swap against the value observed at acquire time,
        ...
        // clobbered by it. Explicit pins (AUTO = false) are never re-pinned here.
            ...
            && trigger is INextVersionTrigger nvtPrefNode)
        {
            ...
            bool rawPreferredNodeAuto = nvtPrefNode.IsPreferredNodeAuto;
            ...
            if (claimUnpinned || stealFromStaleNode)
            {
                ...
                if (claimed > 0)
                {
                    ...
                    nvtPrefNode.SetPreferredNodeRaw(InstanceId, auto: true, markDirty: false);
                }
        ...
        if (failedInstances.Count > 0)
        {
            ...
            try
            {
                foreach (SchedulerStateRecord rec in failedInstances)
                {
                    ...
                    {
                        TimeSpan elapsed = SystemTime.UtcNow() - rec.CheckinTimestamp;
                    ...
                    foreach (FiredTriggerRecord ftRec in firedTriggerRecs)
                    {
                        ...
                        if (canDeferRecovery
                            && ftRec.FireInstanceState!.Equals(StateExecuting)
                            && ftRec.JobDisallowsConcurrentExecution)
                        {
                            preservedFireInstanceIds ??= new HashSet<string>();
                            preservedFireInstanceIds.Add(ftRec.FireInstanceId!);
                            deferredCount++;
                            Log.Info(
                                "ClusterManager: Deferring recovery of [DisallowConcurrentExecution] job '" + jKey +
                                "' (fired trigger '" + ftRec.FireInstanceId + "') — may still be executing on instance '" +
                    ...
                    if (rec.SchedulerInstanceId.Equals(InstanceId) == false)
                    {
                        ...
                        {
                            // Sticky failover: re-pin only AUTO-CLAIMED triggers from the dead node
                            // (PREFERRED_NODE_AUTO = true); explicit pins are left untouched so the
                            // original node reclaims them when it returns.
                                ...
                                && !suppressPreferredNodeAcquisition)
                            {
                                ...
                                int repinned = await nextVersionDelegate.RepinTriggersFromDeadNode(
        ...
        CancellationToken cancellationToken = default)
    {
        for (int retry = 1; !shutdown; retry++)
        {
            ...
            {
                // A configuration error is fatal, not transient — retrying it would loop forever
                // and never fire a trigger. Propagate it so it surfaces to the caller instead of
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 1; attempt <= totalAttempts; attempt++)
        {
            ...
            catch (JobPersistenceException jpe)
            {
                ...
                if (attempt < totalAttempts && IsTransient(jpe))
                {
                    Log.WarnException($"Transient exception on attempt {attempt} of {totalAttempts} in ExecuteInNonManagedTXLock, will retry after {TransientRetryInterval}", jpe);
                }
                else
                {
                    throw;
        ...
        throw new InvalidOperationException("ExecuteInNonManagedTXLock retry loop exited unexpectedly");
    }
}