...
public class MySQLDelegate : StdAdoDelegate
{
    ...
    /// <summary>
    /// The misfire recovery statement carries a FORCE INDEX hint pointing at IDX_*_T_NFT_ST — the
    /// acquisition index, not IDX_*_T_NFT_ST_MISFIRE. The hint attaches to the aliased TRIGGERS table,
    /// since this statement joins the type tables onto it rather than selecting from TRIGGERS alone.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// on NEXT_FIRE_TIME, orders by NEXT_FIRE_TIME ASC, PRIORITY DESC and keeps MISFIRE_INSTR as a
    /// residual — which is exactly the acquisition index's shape since
    /// <see href="https://github.com/quartznet/quartznet/issues/3510">#3510</see> reshaped it, and is
    /// nothing like <c>(SCHED_NAME, MISFIRE_INSTR, NEXT_FIRE_TIME, TRIGGER_STATE)</c>, whose second
    /// column is compared with <c>&lt;&gt;</c> and so stops the seek dead. Every other dialect's
    /// optimizer had already worked this out and picks the acquisition index on its own; MySQL is the
    /// one that cannot, because this hint tells it not to. Measured on a 100,000-trigger table with a
    /// 5,000-row backlog: 15,561 buffer pool reads and 66 ms became 129 reads and 0.7 ms.
    /// </para>
    /// <para>
    ...
    /// </summary>
    /// <remarks>
    /// Its predicate is the recovery statement's without the ORDER BY, so it wants the same index for
    /// the same reason — and it is the cheap peek every misfire-handler pass starts with, so it runs
    /// far more often than the sweep it guards. Measured on the same table with nothing misfired at
    /// all: 4,594 buffer pool reads and 111 ms became 8 reads and 0.7 ms.
    /// </remarks>
