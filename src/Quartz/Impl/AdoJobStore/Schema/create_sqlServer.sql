...
BEGIN
  CREATE TABLE {0}FIRED_TRIGGERS (
    ...
    EXECUTION_GROUP nvarchar(200) NULL,
    PROGRESS int NULL,
    PROGRESS_MESSAGE nvarchar(250) NULL,
...
--;;
-- {0}EXECUTION_HISTORY
IF OBJECT_ID(N'{0}EXECUTION_HISTORY', N'U') IS NULL
BEGIN
  CREATE TABLE {0}EXECUTION_HISTORY (
    SCHED_NAME nvarchar(120) NOT NULL,
    ENTRY_ID nvarchar(140) NOT NULL,
    INSTANCE_NAME nvarchar(200) NOT NULL,
    JOB_NAME nvarchar(150) NOT NULL,
    ...
    CONSTRAINT PK_{1}EXECUTION_HISTORY PRIMARY KEY (SCHED_NAME,ENTRY_ID)
  );
END
--;;
-- {0}MISFIRE_HISTORY
IF OBJECT_ID(N'{0}MISFIRE_HISTORY', N'U') IS NULL
BEGIN
  CREATE TABLE {0}MISFIRE_HISTORY (
    SCHED_NAME nvarchar(120) NOT NULL,
    ENTRY_ID nvarchar(140) NOT NULL,
    INSTANCE_NAME nvarchar(200) NOT NULL,
    TRIGGER_NAME nvarchar(150) NOT NULL,
    ...
    CONSTRAINT PK_{1}MISFIRE_HISTORY PRIMARY KEY (SCHED_NAME,ENTRY_ID)
  );
END
--;;
-- IDX_{1}J_G_N
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_{1}EH_FIRED_TIME' AND object_id = OBJECT_ID('{0}EXECUTION_HISTORY'))
BEGIN
  CREATE INDEX IDX_{1}EH_FIRED_TIME ON {0}EXECUTION_HISTORY(SCHED_NAME, FIRED_TIME);
END
--;;
-- IDX_{1}EH_INST
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_{1}EH_INST' AND object_id = OBJECT_ID('{0}EXECUTION_HISTORY'))
BEGIN
  CREATE INDEX IDX_{1}EH_INST ON {0}EXECUTION_HISTORY(SCHED_NAME, INSTANCE_NAME);
END
--;;
-- IDX_{1}MH_MISFIRE_TIME
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_{1}MH_MISFIRE_TIME' AND object_id = OBJECT_ID('{0}MISFIRE_HISTORY'))
BEGIN
  CREATE INDEX IDX_{1}MH_MISFIRE_TIME ON {0}MISFIRE_HISTORY(SCHED_NAME, MISFIRE_TIME);
END
--;;
-- IDX_{1}MH_INST
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_{1}MH_INST' AND object_id = OBJECT_ID('{0}MISFIRE_HISTORY'))
BEGIN
  CREATE INDEX IDX_{1}MH_INST ON {0}MISFIRE_HISTORY(SCHED_NAME, INSTANCE_NAME);
END
