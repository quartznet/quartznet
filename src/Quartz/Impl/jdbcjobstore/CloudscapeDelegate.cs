...
namespace Quartz.impl.jdbcjobstore
{
	...
	/// </author>
	public class CloudscapeDelegate : StdJDBCDelegate
	{
		/// <summary> <p>
		...
		/// </param>
		public CloudscapeDelegate(ILog log, String tablePrefix, String instanceId) : base(log, tablePrefix, instanceId)
		{
		}
		...
		public CloudscapeDelegate(ILog log, String tablePrefix, String instanceId, ref Boolean useProperties) : base(log, tablePrefix, instanceId, useProperties)
		{
		}
		...
		protected internal override Object getObjectFromBlob(OleDbDataReader rs, String colName)
		{
			...
			sbyte[] inputBytes = rs.getBytes(colName);

			if (null != inputBytes)
			{
				MemoryStream bais = new MemoryStream(SupportClass.ToByteArray(inputBytes));
				...
				BinaryReader in_Renamed = new BinaryReader(bais);
