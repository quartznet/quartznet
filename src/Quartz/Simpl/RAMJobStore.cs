...
namespace Quartz.Simpl
{
    ...
    public class RAMJobStore : IJobStore
    {
        ...
        public virtual bool RemoveCalendar(string calName)
        {
            ...
            lock (lockObject)
            {
                numRefs += triggers.Select(triggerWrapper => triggerWrapper.trigger).Count(trigg => trigg.CalendarName != null && trigg.CalendarName.Equals(calName));
            }
        ...
        public virtual IList<IOperableTrigger> GetTriggersForJob(JobKey jobKey)
        {
            ...
            lock (lockObject)
            {
                trigList.AddRange(from tw in triggers where tw.jobKey.Equals(jobKey) select (IOperableTrigger) tw.trigger.Clone());
            }
        ...
        protected virtual List<TriggerWrapper> GetTriggerWrappersForJob(JobKey jobKey)
        {
            ...
            lock (lockObject)
            {
                trigList.AddRange(triggers.Where(tw => tw.jobKey.Equals(jobKey)));
            }
        ...
        protected virtual List<TriggerWrapper> GetTriggerWrappersForCalendar(string calName)
        {
            ...
            lock (lockObject)
            {
                trigList.AddRange(from tw in triggers let tcalName = tw.trigger.CalendarName where tcalName != null && tcalName.Equals(calName) select tw);
            }
        ...
        public virtual void PauseTrigger(TriggerKey triggerKey)
        {
            lock (lockObject)
            {
                ...
                tw.state = tw.state == InternalTriggerState.Blocked ? InternalTriggerState.PausedAndBlocked : InternalTriggerState.Paused;
                timeTriggers.Remove(tw);
            }
        }
        ...
        public virtual IList<string> PauseJobs(GroupMatcher<JobKey> matcher)
        {
            ...
            lock (lockObject)
            {
                ...
                {
                    pausedGroups.AddRange(jobsByGroup.Keys.Where(@group => op.Evaluate(@group, matcher.CompareToValue)).Where(@group => pausedJobGroups.Add(@group)));
                }
        ...
        public virtual void ResumeTrigger(TriggerKey triggerKey)
        {
            lock (lockObject)
            {
                ...
                tw.state = blockedJobs.Contains(trig.JobKey) ? InternalTriggerState.Blocked : InternalTriggerState.Waiting;
