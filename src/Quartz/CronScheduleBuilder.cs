...
{
    /// <summary>
    /// Returns <c>true</c> if this builder has a deferred H expression that needs a hash key.
    /// </summary>
    bool RequiresHashKey { get; }

    /// <summary>
    /// Resolves deferred H (hash) tokens using the provided trigger key as the hash seed.
    /// </summary>
...
public class CronScheduleBuilder : ScheduleBuilder<ICronTrigger>, IHashKeyAwareScheduleBuilder
{
    ...
    /// <remarks>
    /// <para>
    /// If the expression contains H (hash) tokens, resolution is deferred until
    /// the trigger identity is available (via <see cref="TriggerBuilder"/>).
    /// Use <see cref="CronScheduleWithHash(string, string)"/> to resolve immediately
    /// with an explicit hash key.
    /// </para>
    /// </remarks>
    ...
    /// using the given hash key. Use this when constructing schedules outside of
    /// <see cref="TriggerBuilder"/>, or when you want to use a custom hash key
...
public static class CronScheduleTriggerBuilderExtensions
{
    ...
    /// <param name="cronExpression">Cron expression that may contain H tokens</param>
    /// <param name="hashKey">A stable string key used to derive hash values</param>
    ...
    /// <param name="cronExpression">Cron expression that may contain H tokens</param>
    /// <param name="hashKey">A stable string key used to derive hash values</param>
    /// <param name="action">additional configuration of the schedule</param>
    /// <returns>the updated TriggerBuilder</returns>
