...
public class DBConnectionManager : IDbConnectionManager
{
    ...
    {
        var logger = new MicrosoftLoggingProvider(loggerFactory).GetLogger(nameof(DBConnectionManager));
        log = new LoggerExecutionWrapper(logger, () => LogProvider.IsDisabled);
    }
    ...
    public IDbProvider GetDbProvider(string dataSourceName)
    {
        ...
        {
            throw new ArgumentException("DataSource name cannot be null or empty", nameof(dataSourceName));
        }

        if (!providers.TryGetValue(dataSourceName, out var provider))
        {
            throw new SchedulerException($"There is no DataSource named '{dataSourceName}'");
        }
