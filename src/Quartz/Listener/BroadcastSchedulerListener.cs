...
public sealed class BroadcastSchedulerListener : ISchedulerListener
{
    ...
    public BroadcastSchedulerListener(IEnumerable<ISchedulerListener> listeners)
        : this() => this.listeners.AddRange(listeners);

    public void AddListener(ISchedulerListener listener) => listeners.Add(listener);

    public bool RemoveListener(ISchedulerListener listener) => listeners.Remove(listener);

    public IReadOnlyList<ISchedulerListener> GetListeners() => listeners;

    public ValueTask JobAdded(IJobDetail jobDetail, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.JobAdded(jobDetail, cancellationToken), nameof(JobAdded));

    public ValueTask JobDeleted(JobKey jobKey, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.JobDeleted(jobKey, cancellationToken), nameof(JobDeleted));

    public ValueTask JobScheduled(ITrigger trigger, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.JobScheduled(trigger, cancellationToken), nameof(JobScheduled));

    public ValueTask JobUnscheduled(TriggerKey triggerKey, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.JobUnscheduled(triggerKey, cancellationToken), nameof(JobUnscheduled));

    public ValueTask TriggerFinalized(ITrigger trigger, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.TriggerFinalized(trigger, cancellationToken), nameof(TriggerFinalized));

    public ValueTask TriggersPaused(string? triggerGroup, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.TriggersPaused(triggerGroup, cancellationToken), nameof(TriggersPaused));

    public ValueTask TriggerPaused(TriggerKey triggerKey, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.TriggerPaused(triggerKey, cancellationToken), nameof(TriggerPaused));

    public ValueTask TriggersResumed(string? triggerGroup, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.TriggersResumed(triggerGroup, cancellationToken), nameof(TriggerResumed));

    public ValueTask SchedulingDataCleared(CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.SchedulingDataCleared(cancellationToken), nameof(SchedulingDataCleared));

    public ValueTask TriggerResumed(TriggerKey triggerKey, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.TriggerResumed(triggerKey, cancellationToken), nameof(TriggerResumed));

    public ValueTask JobInterrupted(JobKey jobKey, CancellationToken cancellationToken = new CancellationToken()) =>
        IterateListenersInGuard(l => l.JobInterrupted(jobKey, cancellationToken), nameof(JobInterrupted));

    public ValueTask JobsPaused(string jobGroup, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.JobsPaused(jobGroup, cancellationToken), nameof(JobsPaused));

    public ValueTask JobPaused(JobKey jobKey, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.JobPaused(jobKey, cancellationToken), nameof(JobPaused));

    public ValueTask JobsResumed(string jobGroup, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.JobsResumed(jobGroup, cancellationToken), nameof(JobsResumed));

    public ValueTask JobResumed(JobKey jobKey, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.JobResumed(jobKey, cancellationToken), nameof(JobResumed));

    public ValueTask SchedulerError(string msg, SchedulerException cause, CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.SchedulerError(msg, cause, cancellationToken), nameof(SchedulerError));

    public ValueTask SchedulerStarted(CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.SchedulerStarted(cancellationToken), nameof(SchedulerStarted));

    public ValueTask SchedulerStarting(CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.SchedulerStarting(cancellationToken), nameof(SchedulerStarting));

    public ValueTask SchedulerInStandbyMode(CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.SchedulerInStandbyMode(cancellationToken), nameof(SchedulerInStandbyMode));

    public ValueTask SchedulerShutdown(CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.SchedulerShutdown(cancellationToken), nameof(SchedulerShutdown));

    public ValueTask SchedulerShuttingdown(CancellationToken cancellationToken = default) =>
        IterateListenersInGuard(l => l.SchedulerShuttingdown(cancellationToken), nameof(SchedulerShuttingdown));
