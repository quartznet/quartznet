...
public class SmokeTestPerformer
{
    ...
    private async Task TestGetTriggerStateBlockedWhileExecuting(IScheduler scheduler)
    {
        ...
        try
        {
            ...
            Assert.IsTrue(await jobStarted.WaitAsync(TimeSpan.FromSeconds(10)), "Job should have started within 10 seconds");

            // While the job is executing, GetTriggerState should return Blocked, not Complete
            var state = await scheduler.GetTriggerState(trigger.Key);
            Assert.AreEqual(TriggerState.Blocked, state,
            ...
            Assert.AreEqual(TriggerState.None, finalState,
