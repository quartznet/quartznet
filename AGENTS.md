...
- `database/migrations/4.2/` and `4.3/` are the schema moves since 4.0; a 4.x migration adds nullable columns only, so the previous minor's nodes run beside the next's — a continuation is an `AWAITING` trigger its parent's completion settles in its transaction.
