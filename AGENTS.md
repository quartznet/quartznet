...
- **A migration both branches can run must stay byte-identical on `main` and `3.x`**, `database/README.md` included. It lands on both in a companion PR pair, or a documented path 404s on whichever branch lacks it — which is what #3218 reported. **A 4.x-only schema change is the exception**: it lives only in `database/migrations/4.0/`, ships on `main` with no 3.x companion, because a companion would hand 3.x a script for a feature 3.x does not have. `database/README.md` carries a **Branch** column naming the branch each version section applies to, so a reader can tell the two apart; keep it accurate, since nothing in CI checks cross-branch identity (`VerifyMigrations` only compares a branch's scripts with its own generator). `database/tables/` is the *current* schema and differs by design.
