...
| Added | What it is |
|---|---|
| `QuartzHealthCheckOptions.StaleFiringTolerance` | `double?`, `null` — the reading is off unless asked for. Set, the health check reports *degraded* when a schedulable trigger's fire time has passed by more than this many of the store's own misfire thresholds, and *unhealthy* at twice that, with the overdue trigger's key and the instant it was due in the report's data. This is the silent stall: a scheduler whose thread has wedged is still `Running` and its store still answers, so nothing the check asked before could see it. A scheduler in standby is not asked, and a paused one has no schedulable trigger to be late. `3` is the value to start from — the misfire threshold is also the database store's default misfire-sweep interval, so anything under that reads ordinary recovery as a stall. See [Health checks](packages/hosted-services-integration.md#health-checks) |
| `TriggerQuery.NextFireTimeBefore` | `DateTimeOffset?`, `null` — matches every trigger. Set, it selects the triggers due to fire before that instant, which with an instant in the past is the overdue set; a trigger with no next fire time never matches, because `NEXT_FIRE_TIME < @cutoff` is unknown rather than true for a null column. A non-`required` `init` property on the query record, so a `TriggerQuery` written for 4.1 means exactly what it meant. The HTTP API takes it as `nextFireTimeBefore` on `GET …/triggers` and `HttpScheduler` sends it, so a remote scheduler filters rather than silently ignoring it |
| `Quartz.nupkg` carries an analyzer | Under `analyzers/dotnet/cs`, so referencing `Quartz` is all it takes. It reads a cron expression written as a literal or a `const` with the parser that would read it at run time and fails the build on one that cannot parse (`QZ0001`), reads a `[JobTimeout]` argument the way the attribute's constructor does (`QZ0002`), warns about a job that persists its data map and allows concurrent firings (`QZ0003`), and points out a job body that awaits or loops without ever reading its cancellation token (`QZ0004`). Nothing else in the package changed and no dependency was added. An existing application can see a new build error where it had a run-time exception waiting; `dotnet_diagnostic.QZ0001.severity` moves any one of the four, and `<DisableQuartzAnalyzers>true</DisableQuartzAnalyzers>` in the project file turns the analyzer and the generator off — `ExcludeAssets="analyzers"` does not, on the .NET 10 SDK. See [Compile-Time Checks](tutorial/compile-time-checks.md) |
...
| `AdoConstants.ColumnContinuesTriggerName`, `ColumnContinuesTriggerGroup`, `ColumnContinuationCondition` | The three nullable `QRTZ_TRIGGERS` columns `4.2/add_continuations_<db>.sql` adds: the parent's trigger key and the condition, stored as its integer. `null` on every trigger that is not a continuation |
...
| `SchedulerOrigin.Window = 3` | Appended, as `Remote = 2` was in 4.1. A scheduler reached through the database it shares with the process that runs it. The enum is numeric on the wire, so a **4.1** client reading a **4.2** listing sees a value it has no name for if a window is in the page — exactly as a 4.0 client did when `Remote` was added |
| `SchedulerRegistration.Target` | `string?`, `null` for a scheduler of this process. The attached store a window was found in; with `Name` it is the window's identity, spelled `prod/reporting`. A non-positional `init` property, so the record's constructor is unchanged. It rides on the HTTP API's scheduler listing as `target`, which is now written for every entry — `null` for a scheduler of the answering process, as every other nullable field in that contract is |
| `SchedulerHeaderDto.Target`, `IsWindow`, `DisplayName` | The dashboard's view of the same thing: `Target` is `SchedulerRegistration.Target` carried through, `IsWindow` reads `Origin == SchedulerOrigin.Window`, and `DisplayName` is the label a page shows — `prod/reporting` for a window, the bare name otherwise. A label only: every client member and authorization resource still takes `SchedulerName` |
| `IDriverDelegate.SelectSchedulerNames` | Every `SCHED_NAME` the database holds anything under — the discovery query, and the one member of that interface not scoped to a single scheduler. A **default interface member** that throws `NotSupportedException`: there is no cross-scheduler statement for a default body to borrow, and answering with nothing would show an empty dashboard for a database full of schedulers. `StdAdoDelegate` implements it in dialect-neutral SQL, so every delegate Quartz ships has it |
...
| `QuartzHttpApiOptions.IsJobTypeAllowed` | `Func<string, bool>?` — which job types a request may name. Null, the default, allows every one, so nothing changes for a deployment that does not set it. The predicate is given the type *name* as the request spelled it, never a resolved `Type`: a name arriving over HTTP is data until the side that runs the job loads it. A refused name is `403` with problem details naming the type, and the refusal is per request, so `schedule-multiple` stores none of its batch when one job in it is refused. This is the narrowing for [what `SECURITY.md` says is not a vulnerability](https://github.com/quartznet/quartznet/blob/main/.github/SECURITY.md#what-is-not-a-vulnerability): an authorized caller could name any `IJob`, `NativeJob` among them. See [Narrowing which job types may be named](packages/http-api.md#narrowing-which-job-types-may-be-named) |
...
| `SchedulerOrigin.Remote` | A third origin, for a scheduler reached through a proxy — an `HttpScheduler` registered with `AddQuartzHttpClient`. It used to be reported as `Runtime`, which said only that no registration of this container owned it; `Remote` says the scheduler is in another process, which is what decides whether an action lands here, where its history is kept, and whether this process can stream its events. The enum travels over the wire as its **name**, so a **4.0** client parsing `GET …/schedulers` from a **4.1** host throws on `"Remote"` — which only arises when the host itself fronts a proxy. Upgrade the client, or do not front proxies from a host older clients read |
| `SchedulerRegistration.SchedulerInstanceId` | Which node the scheduler behind a registration is, or `null` when nothing has built one — or when the one that exists could not be asked. Additive, an `init` property beside the positional three. It is carried by the registration because reading `IScheduler.SchedulerInstanceId` off a scheduler in another process is a blocking round trip, and a listing is a page render |
| `IExecutionHistoryStore` | Where what a scheduler has run and what it has missed is kept — `Quartz.Extensibility`, five members: `AddExecution`, `QueryExecutions`, `AddMisfire`, `QueryMisfires`, `CountMisfires`. A job store holds what is *scheduled*; this holds what *happened*, which nothing in Quartz kept on its own outside the dashboard's own in-process store. The shipped implementation is in memory and bounded; register your own before `AddQuartzExecutionHistory()` to keep history somewhere that survives a restart. The dashboard's 4.0 seam, `IDashboardHistoryStore`, still works and is adapted onto this one in both directions |
| `ExecutionHistoryEntry`, `MisfireHistoryEntry` | What the two feeds carry, field for field as the dashboard's `DashboardHistoryEntry` and `DashboardMisfireEntry` do — except that a misfire's job is a `JobKey` rather than the dashboard's `JobKeyDto` |
| `ExecutionHistoryQuery`, `MisfireHistoryQuery` | `PagedQuery` records: a required `SchedulerName`, an optional `SchedulerInstanceId` for one node, and `JobContains` / `TriggerContains`, which match a key's group, its name, or the two joined as `group.name`. Named for what they select on, as every other Quartz filter is — the dashboard's own queries spell the same two filters `JobFilter` and `TriggerFilter` and are unchanged |
...
* **`HttpScheduler.UpdateTriggerDetails` works** rather than throwing `NotSupportedException`. It was the
  ...
  upgrade the scheduler's host before the client that calls this. See
  [Editing a trigger in place](packages/http-api.md#editing-a-trigger-in-place).
...
| `ActivityTags.ExecutionGroup`, `.JobStoreOperation`, `.JobStoreLock`, `.RecoveredInstanceId` added | The four new attribute names, as constants beside the ones that were already there |
...
| `quartz.jobstore.lock.wait.duration` measures every attempt to take a job store lock | Tagged with `quartz.jobstore.lock` — `TRIGGER_ACCESS` or `STATE_ACCESS` — and with `error.type` when the acquisition failed. It is the one measurement a stalled scheduler produces: a lock statement blocked behind another session records no operation and reports no failure. A re-entrant acquisition waited for nothing and is not recorded |
...
| `LockHandlerContext` also carries `TimeProvider`, `CommandTimeout` and `LockWaitWarningThreshold` | The environment a handler locks in, beside the identity it locks under. `DbLockHandler` exposes the clock as a `protected TimeProvider` and both shipped row-lock handlers back off on it, so a retry is observable without waiting out the real second; the threshold is how long one acquisition may go on before `DbLockHandler` logs warning 3716 about it |
...
| `AdoJobStoreOptions.CommandTimeout` added | Bounds every statement the store issues, the lock handler's included; it reaches them through `DriverDelegateContext.CommandTimeout` and `LockHandlerContext.CommandTimeout`. Unset keeps each provider's own default, so nothing changes for a store that does not set it. 3.22 added the same setting to the 3.x line as `quartz.jobStore.commandTimeout`, in milliseconds, and that key translates — with `0`, which is 3.x's "leave the provider's default alone", mapping to the option left unset, because 4.x refuses a non-positive timeout |
| `AdoJobStoreOptions.LockWaitWarningThreshold` added | How long one attempt to take a job store lock may go on before warning **3716** is logged about it, once per acquisition. Defaults to 30 seconds; `null` turns it off and zero is refused. It reports and nothing more — `CommandTimeout`, or a wait timeout in `SelectWithLockSql`, is what ends the wait. Nothing on 3.x reported this at all, so there is no key to translate — see [A Lock Held by a Connection That Is Gone](../troubleshooting.md#a-lock-held-by-a-connection-that-is-gone) |
...
| rc.1 | `Quartz.Dashboard.Services.JobDetailDto`'s `ConcurrentExecutionDisallowed` and `PersistJobDataAfterExecution` are `bool?`, as are the same two fields in every HTTP API job body. Present means stated, absent means "whatever the type says" — a request that omitted the field used to store a `[DisallowConcurrentExecution]` job as safe to run concurrently | [A job's two attribute flags are nullable on the wire](#a-job-s-two-attribute-flags-are-nullable-on-the-wire) |
| rc.1 | A job type name that arrived over the wire is never resolved while a DTO is converted, on either side. A client can schedule a job whose type only the server has, and a job whose type resolves nowhere reads as `200` with both flags absent rather than a permanent `500` | [A job type name is never resolved by the contract types](#a-job-type-name-is-never-resolved-by-the-contract-types) |
| rc.1 | A job type named by a string is checked against `IJob` before anything constructs it. A firing of a type that is not one fails with a `JobInstantiationException` naming it, where it used to be an `InvalidCastException` after the constructor had run | [A named type is checked against `IJob` before it is constructed](#a-named-type-is-checked-against-ijob-before-it-is-constructed) |
...
| rc.1 | `IScheduler`'s **read** members raise `ArgumentNullException` too, which beta.1 left alone; `CronExpression.ResolveHash` raises it where it raised `ArgumentException` | [Null arguments raise `ArgumentNullException`](#null-arguments-raise-argumentnullexception) |
| rc.1 | `IScheduler.Interrupt(jobKey)` raises `JobInterrupted` once per firing it cancelled, where it raised one notification however many it stopped. The new overload naming the fire instance is a default interface member, so an existing listener keeps working | [`JobInterrupted` says which firing was interrupted](#jobinterrupted-says-which-firing-was-interrupted) |
| rc.1 | `SendMailJob` refuses to send when the registered credential answers for every host — a bare `NetworkCredential` — and the host comes from job data. Register a `CredentialCache` bound to the server. `SendMailOptions.EnableSsl` is new and off by default | [The SMTP password does not belong in job data](#the-smtp-password-does-not-belong-in-job-data) |
| rc.1 | `DirectoryScanJob` finds its listener among what the application registered rather than by sweeping every loaded assembly. `AddSingleton<InboxListener>()` alone no longer resolves: register it as `IDirectoryScanListener`, key it, or put it in the `SchedulerContext` | [`DirectoryScanJob` finds its listener among what you registered](#directoryscanjob-finds-its-listener-among-what-you-registered) |
| rc.1 | `DirectoryScanJob` stores its previous scan as a `Dictionary<string, string>` rather than a `List<FileInfo>`, which no serializer could write. Its first firing against a persistent store used to fail to persist | [`DirectoryScanJob` stores its file list as something a job store can write](#directoryscanjob-stores-its-file-list-as-something-a-job-store-can-write) |
