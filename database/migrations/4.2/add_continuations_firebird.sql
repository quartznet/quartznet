...
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$RELATION_FIELDS
                 WHERE TRIM(RDB$RELATION_NAME) = 'QRTZ_TRIGGERS'
                   AND TRIM(RDB$FIELD_NAME) = 'CONTINUES_TRIGGER_NAME')) THEN
    EXECUTE STATEMENT 'ALTER TABLE QRTZ_TRIGGERS ADD CONTINUES_TRIGGER_NAME VARCHAR(150) DEFAULT NULL';
END^
SET TERM ; ^
COMMIT;
...
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$RELATION_FIELDS
                 WHERE TRIM(RDB$RELATION_NAME) = 'QRTZ_TRIGGERS'
                   AND TRIM(RDB$FIELD_NAME) = 'CONTINUES_TRIGGER_GROUP')) THEN
    EXECUTE STATEMENT 'ALTER TABLE QRTZ_TRIGGERS ADD CONTINUES_TRIGGER_GROUP VARCHAR(150) DEFAULT NULL';
END^
SET TERM ; ^
COMMIT;
...
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$RELATION_FIELDS
                 WHERE TRIM(RDB$RELATION_NAME) = 'QRTZ_TRIGGERS'
                   AND TRIM(RDB$FIELD_NAME) = 'CONTINUATION_CONDITION')) THEN
    EXECUTE STATEMENT 'ALTER TABLE QRTZ_TRIGGERS ADD CONTINUATION_CONDITION INTEGER DEFAULT NULL';
END^
SET TERM ; ^
COMMIT;
