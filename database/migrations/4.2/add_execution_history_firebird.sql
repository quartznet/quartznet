...
--        ../4.0/schema_30_to_40_upgrade_firebird.sql first; this file is what 4.2 adds
--        on top of it.
--
...
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$RELATIONS
                 WHERE TRIM(RDB$RELATION_NAME) = 'QRTZ_EXECUTION_HISTORY')) THEN
    EXECUTE STATEMENT 'CREATE TABLE QRTZ_EXECUTION_HISTORY (SCHED_NAME VARCHAR(120) NOT NULL, ENTRY_ID VARCHAR(140) NOT NULL, INSTANCE_NAME VARCHAR(200) NOT NULL, JOB_NAME VARCHAR(150) NOT NULL, JOB_GROUP VARCHAR(150) NOT NULL, TRIGGER_NAME VARCHAR(150) NOT NULL, TRIGGER_GROUP VARCHAR(150) NOT NULL, FIRED_TIME BIGINT NOT NULL, RUN_TIME BIGINT NOT NULL, SUCCEEDED SMALLINT NOT NULL, ERROR_MESSAGE VARCHAR(1000) DEFAULT NULL, RETRY_ATTEMPT INTEGER DEFAULT 0 NOT NULL, RETRY_SCHEDULED SMALLINT DEFAULT 0 NOT NULL, CONSTRAINT PK_QRTZ_EXECUTION_HISTORY PRIMARY KEY (SCHED_NAME,ENTRY_ID))';
END^
SET TERM ; ^
COMMIT;
...
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$RELATIONS
                 WHERE TRIM(RDB$RELATION_NAME) = 'QRTZ_MISFIRE_HISTORY')) THEN
    EXECUTE STATEMENT 'CREATE TABLE QRTZ_MISFIRE_HISTORY (SCHED_NAME VARCHAR(120) NOT NULL, ENTRY_ID VARCHAR(140) NOT NULL, INSTANCE_NAME VARCHAR(200) NOT NULL, TRIGGER_NAME VARCHAR(150) NOT NULL, TRIGGER_GROUP VARCHAR(150) NOT NULL, JOB_NAME VARCHAR(150) DEFAULT NULL, JOB_GROUP VARCHAR(150) DEFAULT NULL, MISFIRE_TIME BIGINT NOT NULL, SCHED_TIME BIGINT DEFAULT NULL, CONSTRAINT PK_QRTZ_MISFIRE_HISTORY PRIMARY KEY (SCHED_NAME,ENTRY_ID))';
END^
SET TERM ; ^
COMMIT;
...
EXECUTE BLOCK AS
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$INDICES WHERE TRIM(RDB$INDEX_NAME) = 'IDX_QRTZ_EH_FIRED_TIME')) THEN
    EXECUTE STATEMENT 'CREATE INDEX IDX_QRTZ_EH_FIRED_TIME ON QRTZ_EXECUTION_HISTORY(SCHED_NAME,FIRED_TIME)';
END^
SET TERM ; ^
COMMIT;
...
EXECUTE BLOCK AS
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$INDICES WHERE TRIM(RDB$INDEX_NAME) = 'IDX_QRTZ_EH_INST')) THEN
    EXECUTE STATEMENT 'CREATE INDEX IDX_QRTZ_EH_INST ON QRTZ_EXECUTION_HISTORY(SCHED_NAME,INSTANCE_NAME)';
END^
SET TERM ; ^
COMMIT;
...
EXECUTE BLOCK AS
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$INDICES WHERE TRIM(RDB$INDEX_NAME) = 'IDX_QRTZ_MH_MISFIRE_TIME')) THEN
    EXECUTE STATEMENT 'CREATE INDEX IDX_QRTZ_MH_MISFIRE_TIME ON QRTZ_MISFIRE_HISTORY(SCHED_NAME,MISFIRE_TIME)';
END^
SET TERM ; ^
COMMIT;
...
EXECUTE BLOCK AS
BEGIN
  IF (NOT EXISTS(SELECT 1 FROM RDB$INDICES WHERE TRIM(RDB$INDEX_NAME) = 'IDX_QRTZ_MH_INST')) THEN
    EXECUTE STATEMENT 'CREATE INDEX IDX_QRTZ_MH_INST ON QRTZ_MISFIRE_HISTORY(SCHED_NAME,INSTANCE_NAME)';
END^
SET TERM ; ^
COMMIT;
