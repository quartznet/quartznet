...
--        ../4.0/schema_30_to_40_upgrade_sqlServer.sql first; this file is what 4.2 adds
--        on top of it.
--
...
IF OBJECT_ID(N'[dbo].[QRTZ_EXECUTION_HISTORY]', N'U') IS NULL
BEGIN
  CREATE TABLE [dbo].[QRTZ_EXECUTION_HISTORY] (
    ...
    RETRY_SCHEDULED bit NOT NULL DEFAULT 0,
...
IF OBJECT_ID(N'[dbo].[QRTZ_MISFIRE_HISTORY]', N'U') IS NULL
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_QRTZ_EH_FIRED_TIME' AND object_id = OBJECT_ID('dbo.QRTZ_EXECUTION_HISTORY'))
BEGIN
  CREATE INDEX [IDX_QRTZ_EH_FIRED_TIME] ON [dbo].[QRTZ_EXECUTION_HISTORY](SCHED_NAME, FIRED_TIME);
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_QRTZ_EH_INST' AND object_id = OBJECT_ID('dbo.QRTZ_EXECUTION_HISTORY'))
BEGIN
  CREATE INDEX [IDX_QRTZ_EH_INST] ON [dbo].[QRTZ_EXECUTION_HISTORY](SCHED_NAME, INSTANCE_NAME);
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_QRTZ_MH_MISFIRE_TIME' AND object_id = OBJECT_ID('dbo.QRTZ_MISFIRE_HISTORY'))
BEGIN
  CREATE INDEX [IDX_QRTZ_MH_MISFIRE_TIME] ON [dbo].[QRTZ_MISFIRE_HISTORY](SCHED_NAME, MISFIRE_TIME);
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IDX_QRTZ_MH_INST' AND object_id = OBJECT_ID('dbo.QRTZ_MISFIRE_HISTORY'))
BEGIN
  CREATE INDEX [IDX_QRTZ_MH_INST] ON [dbo].[QRTZ_MISFIRE_HISTORY](SCHED_NAME, INSTANCE_NAME);
END
GO
