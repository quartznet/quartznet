...
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
   WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'QRTZ_EXECUTION_HISTORY' AND COLUMN_NAME = 'EXECUTION_LOG') > 0,
  'SELECT 1',
  'ALTER TABLE QRTZ_EXECUTION_HISTORY ADD COLUMN EXECUTION_LOG LONGTEXT NULL'
));
