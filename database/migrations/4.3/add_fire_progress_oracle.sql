...
BEGIN
  ...
  WHERE table_name = 'QRTZ_FIRED_TRIGGERS' AND column_name = 'PROGRESS';
  IF column_exists = 0 THEN
    EXECUTE IMMEDIATE 'ALTER TABLE QRTZ_FIRED_TRIGGERS ADD (PROGRESS NUMBER(13) NULL)';
  END IF;
END;
/
...
BEGIN
  SELECT COUNT(*) INTO column_exists FROM user_tab_columns
  WHERE table_name = 'QRTZ_FIRED_TRIGGERS' AND column_name = 'PROGRESS_MESSAGE';
  IF column_exists = 0 THEN
    EXECUTE IMMEDIATE 'ALTER TABLE QRTZ_FIRED_TRIGGERS ADD (PROGRESS_MESSAGE VARCHAR2(1000) NULL)';
  END IF;
END;
/
