...
BEGIN
  ...
  BEGIN
    ...
    IF (EXISTS(SELECT 1 FROM RDB$RELATIONS WHERE TRIM(RDB$RELATION_NAME) = 'QRTZ_EXECUTION_HISTORY')) THEN EXECUTE STATEMENT 'DROP TABLE QRTZ_EXECUTION_HISTORY';
    IF (EXISTS(SELECT 1 FROM RDB$RELATIONS WHERE TRIM(RDB$RELATION_NAME) = 'QRTZ_MISFIRE_HISTORY')) THEN EXECUTE STATEMENT 'DROP TABLE QRTZ_MISFIRE_HISTORY';
  END
END^
SET TERM ; ^
COMMIT;
...
-- The acquisition index is three columns here and five everywhere else. Acquisition orders by
-- NEXT_FIRE_TIME ASC, PRIORITY DESC, and a Firebird index is ascending or descending as a whole,
-- so this is the widest shape the server will accept. See docs/documentation/quartz-4.x/db/.
