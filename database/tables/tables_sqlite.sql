...
-- Every Quartz statement filters SCHED_NAME first, so every index leads with it. There is no
-- misfire index: MISFIRE_INSTR only ever appears as <> -1, which cannot bound a btree scan.
