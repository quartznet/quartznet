...
partial class Build
{
    ...
    static List<(string Path, string Content)> BuildMigrationScripts()
    {
        ...
        foreach (string d in Dialects)
        {
            ...
            // --- 4.2: the continuation columns ---
            files.Add(($"4.2/add_continuations_{d}.sql",
                Header(d, "add the continuation columns", "4.2.0", "#3805",
                    [
                        ...
                        $"     ../4.0/schema_30_to_40_upgrade_{d}.sql first; this file is what 4.2 adds",
                        "     on top of it.",
                    ],
                    [
                    ...
                    sqliteNotIdempotent: true)
                + "\n\n" + AddColumn(d, TableTriggers, "CONTINUES_TRIGGER_NAME", ContinuesTriggerName[d])
                + "\n\n" + AddColumn(d, TableTriggers, "CONTINUES_TRIGGER_GROUP", ContinuesTriggerGroup[d])
                + "\n\n" + AddColumn(d, TableTriggers, "CONTINUATION_CONDITION", ContinuationCondition[d])));
    ...
    /// <summary>
    /// The two optional execution-history tables and their indexes.
    /// </summary>
    /// <remarks>
    /// Their shape comes out of the schema model in <c>Build.DatabaseSchema.cs</c> rather than being
    /// written out a second time here, so the tables this migration creates are byte for byte the
    /// tables a fresh install and <c>SchemaProvisioning.CreateIfMissing</c> create.
    /// </remarks>
    static string Build42ExecutionHistoryScript(string dialect)
    {
        List<string> extra =
        [
        ...
        string header = Header(dialect, "add the execution history tables", "4.2.0", "#3771",
            [
                ...
                "",
                "3.x  Not applicable. Upgrading from 3.x means running",
                $"     ../4.0/schema_30_to_40_upgrade_{dialect}.sql first; this file is what 4.2 adds",
                "     on top of it.",
            ],
            extra);
        ...
        [
            "-- === 1. QRTZ_EXECUTION_HISTORY ===\n\n"
                + CreateTable(dialect, TableExecutionHistory, HistoryTableBody(dialect, TableExecutionHistory)),

            "-- === 2. QRTZ_MISFIRE_HISTORY ===\n\n"
                + CreateTable(dialect, TableMisfireHistory, HistoryTableBody(dialect, TableMisfireHistory)),
        ];

        // MySQL carries its indexes inside the CREATE TABLE above, for the reason the header gives.
        if (dialect != "mysql_innodb")
        {
